...
chris_jones_bam_meta_normal$composition<-"Whole Blood"
chris_jones_bam_meta_tumor$composition<-"Solid Tissue"
...
mmc2<-mmc2[-1,]
colnames(chris_jones_bam_meta)<-c("originalname","age_at_diagnosis","disease_type","primary_site","grade","gender","location","case_id","phenotype","id","name","project","sample_id","sample_type","composition")
chris_jones_bam_meta<-cbind(chris_jones_bam_meta[,c("originalname","name","sample_id","sample_type","composition")],mmc2[unlist(lapply(chris_jones_bam_meta$case_id, function(x) grep(x,mmc2$Index_ID))),c(1:9,17:19)])
chris_jones_bam_meta$location<-paste("https://s3.console.aws.amazon.com/s3/buckets/opendipg/pHGG/",chris_jones_bam_meta$name,sep="")
chris_jones_bam_meta$platform<-"WXS"
chris_jones_bam_meta$reference<-"hg19"

colnames(chris_jones_bam_meta)[c(1:12,16)]<-c("originalname","name","sample_id","sample_type","composition","case_id","diagnosis","WHO_2007_Grade","Location_group","primary_site","gender","age_at_diagnosis_years","tumor_descriptor")
chris_jones_bam_meta$Material_type<-gsub(",",";",chris_jones_bam_meta$Material_type)
...
chris_jones_bam_meta<-rbind(chris_jones_bam_meta_normal,chris_jones_bam_meta_tumor)
colnames(chris_jones_bam_meta)<-c("originalname","gender","diagnosis","sample_type_v1","location","age_at_diagnosis","grade","ENA","NA","id","name","project","sample_id","sample_type","composition")
chris_jones_bam_meta$originalname<- gsub("HSJD_DIPG","HSJD_DIPG_",chris_jones_bam_meta$originalname)

chris_jones_bam_meta<-cbind(chris_jones_bam_meta[,c("originalname","name","sample_id","sample_type","composition")],mmc2[unlist(lapply(chris_jones_bam_meta$originalname, function(x) grep(x,mmc2$PMID_alias))),c(1:9,17:19)])
chris_jones_bam_meta$location<-paste("https://s3.console.aws.amazon.com/s3/buckets/opendipg/pHGG/",chris_jones_bam_meta$name,sep="")
chris_jones_bam_meta$platform<-"WXS"
chris_jones_bam_meta$reference<-"hg19"
...
chris_jones_bam_meta<-rbind(chris_jones_bam_meta_normal,chris_jones_bam_meta_tumor)
colnames(chris_jones_bam_meta)<-c("subject_id","id","name","project","sample_id","sample_type","originalname","gender","disease_type","NA","primary_site","age_at_diagnosis","NA","composition")

chris_jones_bam_meta_RUSL<-chris_jones_bam_meta[grep("RUSL", chris_jones_bam_meta$originalname),]
chris_jones_bam_meta<-cbind(chris_jones_bam_meta[-grep("RUSL", chris_jones_bam_meta$originalname),c("originalname","name","sample_id","sample_type","composition")],mmc2[unlist(lapply(chris_jones_bam_meta$originalname, function(x) grep(x,mmc2$PMID_alias))),c(1:9,17:19)])
...
chris_jones_bam_meta_RUSL<-data.frame(chris_jones_bam_meta_RUSL[,c("originalname","name","sample_id","composition","sample_type")],'Index_ID'="NA","Diagnosis"="Diffuse intrinsic pontine glioma","WHO_2007_Grade"="4","Location_group"="Brainstem","Location_specific"="Pons","Gender"="Male","Age_at_diagnosis_years"="6.7","Survival_time_months"="16.8","Material_type"="NA","Material_source"="NA","Sampling"="NA")
...
chris_jones_bam_meta$platform<-"WGS"
chris_jones_bam_meta$reference<-"hg19"
...
merged_targets_EMTAB_5528_450K<-cbind(merged_targets_EMTAB_5528_450K[,c("Source.Name","Array.Data.File")],merged_targets_EMTAB_5528_450K[,c(31:38,46:48)])
colnames(merged_targets_EMTAB_5528_450K)[1:2]<-c("originalname","name")
merged_targets_EMTAB_5528_450K$sample_id<-gsub("[.]idat","",merged_targets_EMTAB_5528_450K$name)
merged_targets_EMTAB_5528_450K$location<-paste("https://s3.console.aws.amazon.com/s3/buckets/opendipg/pHGG/",merged_targets_EMTAB_5528_450K$name,sep="")
merged_targets_EMTAB_5528_450K$case_id<-merged_targets_EMTAB_5528_450K$originalname
colnames(merged_targets_EMTAB_5528_450K)[c(1:10,12)]<-c("originalname","name","diagnosis","WHO_2007_Grade","Location_group","primary_site","gender","age_at_diagnosis_years","Survival_time_months","Alive(0)_Dead(1)","tumor_descriptor")
merged_targets_EMTAB_5528_450K$sample_type<-"Tumor"
merged_targets_EMTAB_5528_450K$composition<-"Solid Tissue"
merged_targets_EMTAB_5528_450K$platform<-"450K"
merged_targets_EMTAB_5528_450K$reference<-"hg19"
merged_targets_EMTAB_5528_450K$Material_type<-gsub(", ",";",merged_targets_EMTAB_5528_450K$Material_type)
...
upload_bac32_tumor<-upload_bac32[!is.na(upload_bac32$PMID1),c(1:2,8:17,25:27)]
colnames(upload_bac32_tumor)[1:2]<-c("originalname","name")
...
colnames(upload_bac32_notreportedtumor_blood)<-c("originalname","name","Gender","Diagnosis","Age_at_diagnosis_years","Location_group","originlname","sample_id")
...
upload_bac32_notreportedtumor_blood$Material_type<-"NA"
upload_bac32_notreportedtumor_blood$Sampling<-"NA"
upload_bac32_notreportedtumor_blood$Material_source<-"NA"


upload_bac32_manifest<-rbind(upload_bac32_tumor,upload_bac32_notreportedtumor_blood)
upload_bac32_manifest$location<-paste("https://s3.console.aws.amazon.com/s3/buckets/opendipg/pHGG/",upload_bac32_manifest$name,sep="")
...
upload_bac32_manifest$sample_type<-"Tumor"
upload_bac32_manifest$composition<-"Solid Tissue"
upload_bac32_manifest$platform<-"BAC32K"
upload_bac32_manifest$reference<-"hg19"
upload_bac32_manifest$Material_type<-gsub(", ",";",upload_bac32_manifest$Material_type)
colnames(upload_bac32_manifest)[1:14]<-c("originalname","name","sample_id","case_id","diagnosis","WHO_2007_Grade","Location_group","primary_site","gender","age_at_diagnosis_years","Survival_time_months","Alive(0)_Dead(1)","Material_type","tumor_descriptor")
...
#merge all manifests
all_CJ_ETABM_857_bac32k<-read.csv("~/Documents/CNMC/all_CJ_ETABM_857_bac32k.csv",sep=",")
all_CJ_ETABM_857_bac32k$ACCESS<-"PUBLIC"
all_CJ_EMTAB_5528_450K<-read.csv("~/Documents/CNMC/all_CJ_EMTAB_5528_450K.csv",sep=",")
all_CJ_EMTAB_5528_450K$ACCESS<-"PUBLIC"
all_CJ_EGAD00001000705<-read.csv("~/Documents/CNMC/all_CJ_EGAD00001000705.csv",sep=",")
all_CJ_EGAD00001000705$ACCESS<-"CONTROLLED"
all_CJ_EGAD00001000706<-read.csv("~/Documents/CNMC/all_CJ_EGAD00001000706.csv",sep=",")
all_CJ_EGAD00001000706$ACCESS<-"CONTROLLED"
all_CJ_EGAD00001003762<-read.csv("~/Documents/CNMC/all_CJ_EGAD00001003762.csv",sep=",")
all_CJ_EGAD00001003762$ACCESS<-"CONTROLLED"

all_CJ_2019_May<-rbind(all_CJ_ETABM_857_bac32k,all_CJ_EMTAB_5528_450K,all_CJ_EGAD00001000705,all_CJ_EGAD00001003762,all_CJ_EGAD00001000706)

all_CJ_2019_May$primary_site<-gsub(" - NOS","",all_CJ_2019_May$primary_site)

write.csv(all_CJ_2019_May,"~/Documents/CNMC/all_CJ_2019_May.csv",quote = F,row.names = F)

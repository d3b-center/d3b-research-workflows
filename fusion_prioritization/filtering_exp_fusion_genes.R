...
tximport_genes<-readRDS(paste0(basePD,"/data/interim/txirsem_all_CBTTC_1092_sampleid_aliqoutid.RDS"))
#rna.mat<-tximport_genes$abundance
rna.mat<-tximport_genes
...
nrow(rna.mat)
rna.mat[which(rna.mat$sample_id %in% "7316-114" & rna.mat$gene_short_name == "IRS2"),'value']
...
for(i in 1:nrow(df)){
  ...
  df[i,'Gene1_sample'] <- ifelse(is.na(genea.val) || length(genea.val) == 0, NA, genea.val)
  ...
  df[i,'Gene1_expr'] <- ifelse(df[i,'Gene1_sample'] < df[i,'Gene1_mean'],'Decreased',ifelse(df[i,'Gene1_sample'] == df[i,'Gene1_mean'], 'Same','Increased'))
  df[i,'Gene2_not_expressed'] <- ifelse(length(geneb.expr) == 0, NA, geneb.expr)
  df[i,'Gene2_sample'] <- ifelse(is.na(geneb.val) || length(geneb.val) == 0, NA, geneb.val)
  ...
  df[i,'Gene2_expr'] <- ifelse(df[i,'Gene2_sample'] < df[i,'Gene2_mean'],'Decreased',ifelse(df[i,'Gene2_sample'] == df[i,'Gene2_mean'], 'Same','Increased'))
}
df[is.na(df)] <- NA
